...
`altText` / `blocks` / `actions`）を Write ツールでファイルに書き、次を呼ぶだけ:
...
そのため描画層を「ボタン1つ」から「最大3つ」に拡張し、共通設計書（gas-notify-hub）にも
3.3節として書き足した。**行き先が複数あるなら `actions` に並べること。**
`action`（単数）も後方互換で受ける。両方渡すと連結され、先頭3つまでが footer に並ぶ。
...
`sources` を付けた初回の実行で、ルーティンがリポジトリの `CLAUDE.md` を読み、
「終わるときに HANDOFF.md を書き直してコミット・push」に従って**この引き継ぎを上書きして push した**
（commit `fe0b46f`）。毎朝これが起きると設計の経緯が実行ログで消える。
...
- **ボタンを複数にしたカードは実機で未確認。** ボタン1つのカードは実機で確認済み（見た目は良好）。
  3つ並べたときの見え方は分かっていない
...
1. **ボタンを複数にしたカードを実機で確認する。** 2026-08-27 の1通目（ボタン1つ）は実機で確認済みで
   見た目は良好だった。ボタン3つ並べたときの詰まり具合は未確認
