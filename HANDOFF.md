...
- **プロンプト修正後のルーティン2は1度も走っていない。** 文字数カウントの修正が効いているかは
  8/28 08:40 の初回定時実行で確認する（実行ログの「本文の文字数（python3で数えた値）」を見る）
...
1. **8/28(金)08:40 のルーティン2 初回定時実行を確認する。** 文字数が python3 で数えられていて、
   本文が不要に削られていないこと（`RemoteTrigger list_runs` → `get_run_log`）
