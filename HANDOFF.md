...
**Flex での送信が成功したかは、この引き継ぎを書いた時点では未確認。次のセッションで必ず確認すること**
（下記「次にやること」1）。
...
- **Flex 化後の実機の見え方は未確認。** スクリプトの検証が通ることと、LINE アプリでの
  フォント・行間・折り返しは別物（設計書9節）
- **Flex での送信が実際に 200 を返したか未確認**（上記のとおり）
...
1. **Flex 化後の実行結果を確認する。** LINE を見て、カードで届いているか（テキストに
   フォールバックしていないか）を確かめる。ログは:
   `RemoteTrigger get_run_log` に `session_id=cse_01DCvwob5DYo2V92YeE2Qgky`
...
5. 9/8 以降、ルーティン1が「完全に終了」を垂れ流していないか LINE を見る（告知待ちモードは未検証）
